"""
Vib2Sound + CMC loss training script.
Imports from vib2sound-lab without modifying any file there.

  - Subclasses Vib2Sound_multichannel to expose presigmoid FC2 output needed for AVCBlock. Some extra lines in forward(), nothing else changes.
  - Reuses vib2sound-lab's create_dataloader, Audio, hparams, writer exactly
    as the original trainer.py does.
  - Adds CMC loss alongside the original MSE loss.
  - Checkpoint saves vib2sound weights separately so they stay compatible with the original inference.py.

Batch tuple from dataloader (confirmed from dataloader.py):
    radio1_mag   : (B, T, 193)-> accelerometer bird 1
    radio2_mag   : (B, T, 193)-> accelerometer bird 2
    target_mag   : (B, T, 193)-> clean target spectrogram
    nontarget_mag: (B, T, 193)-> clean nontarget spectrogram
    mixed_mag    : (B, n_mics, T, 193)-> mixture magnitude, all mics
    mixed_phase  : (B, n_mics, T, 193)-> mixture phase, all mics
    target_wav   : (B, n_mics, samples)-> waveform (not used in loss)
    nontarget_wav: (B, n_mics, samples)-> waveform (not used in loss)
...
#Device: CUDA on Linux/Windows, MPS on Apple Silicon, CPU fallback (since i have a Mac i have to use MPS but the lab computer is Windows from what i saw so..CUDA should worok once i run it there hopefully)
...
class Vib2Sound_multichannel_CMC(Vib2Sound_multichannel):
    ...
    def forward(self, x_mixture, x_phase, x_radio1, x_radio2):
        ...
        x_target = F.relu(x_target)
        raw_target = self.fc2(x_target)          ##!! presigmoid
        ...
        x_nontarget = F.relu(x_nontarget)
        raw_nontarget = self.fc2(x_nontarget)    ##!! presigmoid
...
def train_cmc(args, hp, hp_str):
    ...
    if args.chkpt_path:
        args.chkpt_path = os.path.abspath(args.chkpt_path)
    os.chdir(LAB_PATH)
    ...
    if args.chkpt_path is not None:
        ...
        ckpt = torch.load(args.chkpt_path, map_location=device)
    ...
    try:
        while True:
            ...
            for (radio1_mag, radio2_mag, target_mag, nontarget_mag,
                ...
                #move to GPU and cast (= as original train.py)
                radio1_mag    = radio1_mag.to(device).float() # (B, T, 193)
                radio2_mag    = radio2_mag.to(device).float() # (B, T, 193)
                target_mag    = target_mag.to(device).float() # (B, T, 193)
                nontarget_mag = nontarget_mag.to(device).float() # (B, T, 193)
                mixed_mag     = mixed_mag.to(device).float() # (B, n_mics, T, 193)
                mixed_phase   = mixed_phase.to(device).float() # (B, n_mics, T, 193)
                ...
                if step % hp.train.summary_interval == 0:
                    ...
                    #MyWriter subclasses SummaryWriter directly (call add_scalar on it)
                    writer.add_scalar("loss/mse",    loss_mse.item(),        step)
                    writer.add_scalar("loss/cmc",    cmc_out.loss.item(),    step)
                    writer.add_scalar("cmc/cos_pos", cmc_out.cos_pos.item(), step)
                    writer.add_scalar("cmc/cos_neg", cmc_out.cos_neg.item(), step)
                    logger.info(
                        f"step {step:>7d} | "
                        f"total {loss_val:.4f} | "
                ...
                if step % hp.train.checkpoint_interval == 0:
                    ...
                    torch.save({
                        "model":          model.state_dict(),   # should be compatible with original inference.py
...
if __name__ == "__main__":
    ...
    #config.yaml stores data path relative to vib2sound-lab/ (e.g. 'voc_cop_data/copExpBP08') try to resolve it to absolute so the script works regardless of where it's launched from.
